...
def _ensure(pkg, import_as=None, install_args=None):
    module_name = import_as or pkg
    try:
        importlib.import_module(module_name)
    except Exception:
        print(f"  [AUTO-INSTALL] {pkg}...")
        spec = install_args or [pkg]
...
for pkg, imp, install_args in [
    ("ultralytics", "ultralytics", None),
    ("opencv-python", "cv2", None),
    ("numpy", "numpy", ["--upgrade", "--force-reinstall", "numpy"]),
    ("torch", "torch", None),
    ("torchvision", "torchvision", None),
    ("scipy", "scipy", ["--upgrade", "--force-reinstall", "scipy"]),
    ("matplotlib", "matplotlib", ["--upgrade", "--force-reinstall", "matplotlib"]),
    ("Pillow", "PIL", None),
    ("supervision", "supervision", ["--upgrade", "--force-reinstall", "supervision"]),
]:
    _ensure(pkg, imp, install_args)
